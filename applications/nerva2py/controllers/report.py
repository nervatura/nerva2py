...
def python_demo():
  response.view="report/python.html"
  response.title = "Nervatura Report Help & Demo"
  response.subtitle = " Ver.No: "+response.verNo
  return dict()

def javascript_demo():
  response.view="report/javascript.html"
  response.title = "Nervatura Report Help & Demo"
  response.subtitle = " Ver.No: "+response.verNo
  return dict()
...
def document():
  ...
  if request.vars.landscape: orient = "l"
  if request.vars.data: output = "xml"
  if request.vars.html: output = "html"
  if request.vars.py: src = "py"
  return create_report(src, orient, output) 

def template():
  if request.vars.py:
    filename = os.path.join(request.folder, 'static/resources/application/report/python/ntura', 'sample.py')
...
def sample_report(rpt):  
  ...
  items = []
  for i in range(1, 30):
    items.append({"text": str(i)+". Lorem ipsum dolor", "number": "123 456", "date": "2014.01.01"})
