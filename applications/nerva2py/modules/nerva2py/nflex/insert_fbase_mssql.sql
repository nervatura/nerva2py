...
sqlstr=' case when addr.city is null then '''' else addr.city end + '' '' + case when addr.street is null then '''' else addr.street end' 
...
left join customer rf_customer on fv.value = cast(rf_customer.id as nvarchar)
left join tool rf_tool on fv.value = cast(rf_tool.id as nvarchar)
left join trans rf_trans on fv.value = cast(rf_trans.id as nvarchar)
left join product rf_product on fv.value = cast(rf_product.id as nvarchar)
left join project rf_project on fv.value = cast(rf_project.id as nvarchar)
left join employee rf_employee on fv.value = cast(rf_employee.id as nvarchar)
left join place rf_place on fv.value = cast(rf_place.id as nvarchar)
...
UPDATE ui_applview SET sqlstr='select e.id, c.custnumber, c.custname, e.calnumber, eg.groupvalue as eventgroup, 
  cast(e.fromdate as date) as fromdate, 
  SUBSTRING(cast(cast(e.fromdate as time) as nvarchar),0,6) as fromtime, 
  cast(e.todate as date) as todate, 
  SUBSTRING(cast(cast(e.todate as time) as nvarchar),0,6) as totime,
...
update ui_viewfields set 
sqlstr=' SUBSTRING(cast(cast(e.fromdate as time) as nvarchar),0,6)' 
where viewname='CustomerEvents' and fieldname='fromtime';
update ui_viewfields set 
sqlstr=' SUBSTRING(cast(cast(e.todate as time) as nvarchar),0,6)' 
...
UPDATE ui_applview SET sqlstr='select cast(t.id as varchar)+''_''+tg.groupvalue+''_''+dg.groupvalue as id, t.transnumber, t.crdate, case when msd.msg is null then dg.groupvalue else msd.msg end as direction
...
UPDATE ui_applview SET sqlstr='select cast(t.id as varchar)+''_''+tg.groupvalue+''_''+dg.groupvalue as id, case when mst.msg is null then tg.groupvalue else mst.msg end as transtype, 
...
UPDATE ui_applview SET sqlstr='select cast(t.id as varchar)+''_''+tg.groupvalue+''_''+dg.groupvalue as id, 
...
UPDATE ui_applview SET sqlstr='select tg.groupvalue+''_''+dg.groupvalue+''_''+(case when it.id is null then cast(t.id as varchar) else cast(it.id as varchar) end) as id, 
  case when mst.msg is null then tg.groupvalue else mst.msg end as transtype, 
  case when msd.msg is null then dg.groupvalue else msd.msg end as direction,
...
UPDATE ui_applview SET sqlstr='select tg.groupvalue+''_''+dg.groupvalue+''_''+(case when delt.ref_id is null then cast(t.id as varchar) else cast(delt.ref_id as varchar) end) as id, 
  case when mst.msg is null then tg.groupvalue else mst.msg end as transtype, 
  case when msd.msg is null then dg.groupvalue else msd.msg end as direction, t.transnumber, df.description as fielddef,
case when fg.groupvalue in (''notes'', ''string'', ''valuelist'', ''flink'') then fv.value else null end as text_value,
cast(case when fg.groupvalue in (''float'', ''integer'') then fv.value else null end AS REAL) as number_value,
...
UPDATE ui_applview SET sqlstr='select e.id, p.pronumber, p.description as pdescription, e.calnumber, eg.groupvalue as eventgroup, 
  cast(e.fromdate as date) as fromdate, SUBSTRING(cast(cast(e.fromdate as time) as nvarchar),0,6) as fromtime,  
  cast(e.todate as date) as todate, SUBSTRING(cast(cast(e.todate as time) as nvarchar),0,6) as totime,
  e.subject, e.place, e.description
from event e
