...
update ui_reportsources set sqlstr = 'select concat(case when ti.deposit=1 then ''Deposit: '' else '''' end, case when pf.value is null then '''' else concat(pf.value,'' '') end, ti.description) as description
...
[engine sqlite]update ui_reportsources set sqlstr = 'select case when ti.deposit=1 then ''Deposit: '' else '''' end || case when pf.value is null then '''' else pf.value||'' '' end || ti.description as description
, pf.value as vtsz, p.partnumber as partnumber, ti.unit as unit, ti.qty as qty, tax.taxcode as taxcode
, ti.fxprice as fxprice, ti.netamount as netamount, ti.discount as discount, ti.vatamount as vatamount, ti.amount as amount, ti.deposit as deposit 
from item ti inner join product p on p.id = ti.product_id inner join tax on tax.id = ti.tax_id 
left join fieldvalue pf on pf.ref_id = p.id and fieldname = ''szj_vtsz'' 
...
[engine mssql]update ui_reportsources set sqlstr = 'select case when ti.deposit=1 then ''Deposit: '' else '''' end + case when pf.value is null then '''' else pf.value+'' '' end + ti.description as description
, pf.value as vtsz, p.partnumber as partnumber, ti.unit as unit, ti.qty as qty, tax.taxcode as taxcode
, ti.fxprice as fxprice, ti.netamount as netamount, ti.discount as discount, ti.vatamount as vatamount, ti.amount as amount, ti.deposit as deposit 
from item ti inner join product p on p.id = ti.product_id inner join tax on tax.id = ti.tax_id 
left join fieldvalue pf on pf.ref_id = p.id and fieldname = ''szj_vtsz'' 
...
[engine mssql]update ui_reportsources set sqlstr = 'select t.transnumber as transnumber, t.transdate as transdate, t.crdate as crdate
, case when t.notes is null then '''' else t.notes end as notes
, case when t.fnote is null then '''' else t.fnote end as fnote
, case when (tcast.value != ''normal'' ) then 
    case when t.ref_transnumber is null then '''' else ''Ref. invoice: '' + t.ref_transnumber end 
...
update ui_report set report = '<template>
  ...
  <header>
    <row height="10">
      ...
      <cell name="label" value="INVOICE" font-style="BI" font-size="26" color="14212058"/>
  ...
  <details>
    <row>
      <cell name="label" width="50%" value="Supplier" font-style="B" background-color="16119285" border="LT" border-color="14212058"/>
      <cell name="label" value="Customer" font-style="B" background-color="16119285"  border="LRT" border-color="14212058"/>
    </row>
    <row>
    ...
    <row>
      <cell name="label" align="C" width="30" font-style="B" value="Delivery date" background-color="16119285" border="LBT" border-color="14212058"/>
      <cell name="label" align="C" width="30" font-style="B" value="Due date" background-color="16119285" border="LBT" border-color="14212058"/>
      <cell name="label" align="C" width="30" font-style="B" value="Creation date" background-color="16119285" border="LBT" border-color="14212058"/>
      <cell name="label" align="C" width="20" font-style="B" value="Currency" background-color="16119285" border="LBT" border-color="14212058"/>
      <cell name="label" align="C" width="30" font-style="B" value="Payment" background-color="16119285" border="LBT" border-color="14212058"/>
      <cell name="label" align="C" font-style="B" value="Source Ref.No." background-color="16119285" border="LBTR" border-color="14212058"/>
    </row>
    <row>
    ...
    <row>
      <cell name="label" width="30" font-style="B" value="Comments" background-color="16119285" border="LB" border-color="14212058"/>
    ...
    <datagrid name="items" databind="items" border="1" border-color="14212058" font-size="10">
      ...
      <columns>
        ...
        <column width="25%" fieldname="description" label="Description"/>
        <column width="8%" fieldname="unit" label="Unit"/>
        <column width="7%" fieldname="qty" align="R" thousands=" " digit="2" label="Qty"/>
        <column width="10%" fieldname="fxprice" align="R" thousands=" " digit="2" label="Price"/>
        <column width="7%" fieldname="discount" align="R" thousands=" " digit="2" label="Disc.%"/>
        <column width="8%" fieldname="taxcode" align="R" label="VAT%"/>
        <column width="11%" fieldname="netamount" align="R" thousands=" " digit="2" label="NetAmount"/>
        <column width="9%" fieldname="vatamount" align="R" thousands=" " digit="2" label="VAT"/>
        <column width="12%" fieldname="amount" align="R" thousands=" " digit="2" label="Amount"/>
      </columns>  
    </datagrid>
    <vgap height="2"/>
    <datagrid width="50%" name="taxgroup" databind="taxgroup" border="1" border-color="14212058" font-size="10">
      <header background-color="16119285"/>
      <columns>
        <column width="19%" fieldname="taxcode" align="R" label="VAT %" footer="Total"/>
        <column width="27%" fieldname="netamount" align="R" thousands=" " digit="2" label="Netamount" footer="={{head.0.sum_netamount}}"/>
        <column width="27%" fieldname="vatamount" align="R" thousands=" " digit="2" label="VAT" footer="={{head.0.sum_vatamount}}"/>
        <column width="27%" fieldname="amount" align="R" thousands=" " digit="2" label="Amount" footer="={{head.0.sum_amount}}"/>
      </columns>  
    </datagrid>
    <vgap height="4"/>
