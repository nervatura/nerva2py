...
function set_product_value(_product_id, _partnumber, _description, _unit, _tax_id) {
	...
	if (document.getElementById('production_product_id')!=null) {
		if (document.getElementById('frm_head').style.display == 'block') {
...
function set_transitem_value(_trans_id, _transnumber, _transtype, _direction, _curr) {
	if (document.getElementById('edit_link_invoice')!=null) {
		if (document.getElementById('edit_link_invoice').style.display == 'block') {
...
function set_place_value(_place_id, _planumber, _description, _curr) {
	...
	if (document.getElementById('edit_movement')!=null) {
		if (document.getElementById('edit_movement').style.display == 'block') {
...
function set_fieldvalue(_fieldvalue_id, _fieldname, _description, _value, _label, _notes, _fieldtype, _valuelist, _readonly) {
	...
	document.getElementById('frm_head').style.display = 'none';
...
function set_link_invoice(_link_id,_ref_id_1,_ref_id_2,_transnumber,_curr,_amount,_rate){
    ...
    document.getElementById('frm_head').style.display = 'none';
    document.getElementById('edit_link_invoice').style.display = 'block';
...
function set_barcode(_barcode_id,_code,_description,_barcodetype,_qty,_defcode) {
	...
	document.getElementById('frm_head').style.display = 'none';
	document.getElementById('edit_barcode').style.display = 'block';
...
function address_update(){
	document.forms["frm_address"].submit();
}
//--------------------------------------------------
...
function set_movement(_movement_id, _shippingdate, _product_id, _product, _tool_id, _serial, _qty, _notes, _place_id, _planumber, _shared){
	...
	document.getElementById('frm_head').style.display = 'none';
	document.getElementById('edit_movement').style.display = 'block';
	if (document.getElementById("tool_id")) {
...
//--------------------------------------------------
function show_report(back_url,ms_missing){
	...
	if(ctmp.value==''){
		...
		if(confirm(ms_wn_delete)){
			...
			jQuery.ajax({
		    	...
		    	url: surl,
		    	success: function(data) {
		    		ctmp.remove(ctmp.selectedIndex);
		    	}
			});
			}
		};
...
function event_update() {
	if (document.getElementById('tb_description')==null) {
		var tb_description = document.createElement("input");
		tb_description.setAttribute("id", "tb_description");
		tb_description.setAttribute("type", "hidden");
		tb_description.setAttribute("name", "description");
		document.forms["frm_event"].appendChild(tb_description);	
	}
	if (document.getElementById('event_description')!=null) {
		document.getElementById('tb_description').value = document.getElementById('event_description').value;	
	}
...
function create_report(cmd,ms_missing) {
	...
	frm.setAttribute('action',document.getElementById("report_url").value);
	frm.setAttribute('target',"_blank");
	
	frm_values = ["cmb_templates","cmb_orientation","cmb_size","page_copy"];
	for (var i=0;i<frm_values.length;i++) {
		var frm_value = document.createElement("input");
		frm_value.setAttribute("type", "hidden");
		frm_value.setAttribute("name", document.getElementById(frm_values[i]).name);
		frm_value.setAttribute("value", document.getElementById(frm_values[i]).value);
		frm.appendChild(frm_value);
	}
	var frm_value = document.createElement("input");
	frm_value.setAttribute("type", "hidden");
	frm_value.setAttribute("name", "cmd");
	frm_value.setAttribute("value", cmd);
	frm.appendChild(frm_value);
