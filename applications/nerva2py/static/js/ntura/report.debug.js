...
var report = function(orientation, unit, format) {
      ...
      if (typeof self.template.elements.data[dbv[0]]!=="undefined") {
        if (typeof self.template.elements.data[dbv[0]]==="object") {
          if (Array.isArray(self.template.elements.data[dbv[0]])) {
            try {
              if (self.template.elements.data[dbv[0]][parseInt(dbv[1])][dbv[2]]!==null) {
                return self.template.elements.data[dbv[0]][parseInt(dbv[1])][dbv[2]].toString();}
              else {return "";}}
  ...
  function create_datagrid(grid_element) {
        ...
        if (header_options.merge===0) {
          ...
          if (column_options.width!==null) {
            ...
            column_options.width = parseFloat(
              doc.getStringUnitWidth(column_options.label)*doc.internal.getFontSize()/doc.internal.scaleFactor
              +self.template.page.padding*2)+1; 
            zcol += 1;}
    ...
    rows.forEach(function(row, row_index){
      ...
      header_options.columns.forEach(function(column){
        ...
        if (header_options.merge===0) {
          var cheight = get_height(column.text,column.width,self.template.page.padding,grid_options["font-style"]).height;
          if (cheight>grid_options.height){grid_options.height = cheight;}}
        else {
          grid_options.text += " "+column.text;
          self.template.xml.details += "\n      <"+column.fieldname.toString()+"><![CDATA["+column.text.toString()
    ...
    if (header_options.merge===0) {
      footers.forEach(function(column){
        var cheight = get_height(column.text,column.width,self.template.page.padding,footer_options["font-style"]).height;
  ...
  function create_cell(options, header_options){
    ...
    if ((typeof options.padding==="undefined")||(options.padding===null)) {
      options.padding = parseInt(self.template.page.padding);}
    else {options.padding = parseInt(options.padding);}
    if ((typeof options.multiline==="undefined")||(options.multiline===null)) {
      options.multiline = "false";}

    if ((typeof options.ln==="undefined")||(options.ln===null)) {
      options.ln = 0;}
    else {options.ln = parseInt(options.ln);}
          ...
          if (img_src !== null) {
            ...
            if (img_src.toString().substr(0,10)==="data:image") {
              ...
              if (check_page_break(self.template.page.cursor.y, img_height)) {
                doc.addPage();}
              doc.addImage(img_src, null, self.template.page.cursor.x, self.template.page.cursor.y, width, img_height, null, compression);
  ...
  function parse_value(vname, value) {
    switch(vname) {
      ...
      case "merge":
      case "visible-value":
        value = parseInt(get_value(value,0));
        if (value!==0) {return 1;} 
        return value;
          ...
          if (value.toString().charAt(0)==="#") {
            ...
            if(!isNaN(parseInt(value))) {
              if (parseInt(value)>255) {
                value = hex2greyscale(parseInt(value).toString(16).toUpperCase());} 
